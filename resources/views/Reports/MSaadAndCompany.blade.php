...
<head>
    ...
    <title>M Saad and Company Report</title>
    <style>
...
<body>
    ...
    <main>
        <h2 class="text-center">{{$report_type}} Report</h2>
        ...
        @if ($itemStatus == 'delivered' || $itemStatus == '')
        <table>
            <thead>
                <tr>
                    <th class="">Sr#</th>
                    ...
                    <td class="">
                        <table>
                            <tbody>
                                ...
                                <tr>
                                    <td class="w-80">{{$item->quotationItem->tenderItem->item->name}}({{$item->quotationItem->tenderItem->description}}) Qty Left: {{$item->qty_left}}</td>
                                    @if ($amountIncluded == 'amount_included')
                                    <td class="w-20">{{$supplyOrder->quotation?->currency}} {{$item->quotationItem->unit_price}}</td>
                                    @else
                                    <td class="w-20"></td>
                                    @endif
                                </tr>
        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="">{{$supplyOrder->quotation?->tender?->file_name}}</td>
                    <td class="">{{upperCaseAndRemoveUnderscore($supplyOrder->status)}}</td>
                    <td class="">{{$supplyOrder->paymentRecieving ? 'Recieved' : 'Pending'}}</td>
                    <td class="">{{$supplyOrder->quotation?->currency}} {{numberFormate(calculateTaxAndGetTotal($supplyOrder->total_price, $supplyOrder->quotation->tax))}}</td>
                </tr>
                @endforeach
                <tr>
                    <td colspan="7" class="footer" style="border: 0px !important;">Grand Total:</td>
                    <td class="text-right">{{numberFormate($totalAmount)}}</td>
                </tr>
            </tbody>
        </table>
